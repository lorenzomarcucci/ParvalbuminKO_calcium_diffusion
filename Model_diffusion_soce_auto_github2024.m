%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%% main file for the mathematical model publisehd in Acta
%%%%%%%%%%% Physiologica by Marcucci Lorenzo et al. 2024 to simulate the
%%%%%%%%%%% calcium diffusion into a skeletal muscle sarcomere in the
%%%%%%%%%%% presence and absence of parvalbumin. The file
%%%%%%%%%%% diff_diffusion_soce_auto.m will be uploaded but can be sent upon request at
...
sampling_switch=0; %if preallocation is needed
...
V_EF=((n-1-1+0.5)^2-(max(0,n-1-2+0.5))^2)/n^2*V_max/m; %Vol each comparterment 
...
mcu_syl=1;
    ...
    for anls=1:2 %1 is with PV 2 is PV ablated
        hertz=[60];% possible variations on the stimulation frequency
        held=[2];% Seconds to hold the train of stimuli
        if anls == 1
            anl_p=0;
            pvb_p=1; %parvalbumin 
        ...
        P_tot=1500;%1000; %Total Parv microM
        ...
        MB1_tot=0.2*35.0*1e3; %Total first
        MB2_tot=0.2*90.*1e3; %Total second
        Kd_1=2.0; %µM
        Kd_2=1.7; %µM
        ...
        ratio_2=1;
        %note: C&A84 S_tot=31mM, BUT  Murphy e Lamb JP 2009 CalS=36 umoli/litro di fibra
        %wt 70-80 Ca sites/molecule (Mol Biosyst. 2013 Jul 4; 9(7): 1949?1957.)
        %rapportato a V_TC porta a 83000 su 80 e 72000 su 70
        ...
        for freq=1:length(hertz)
            if anls == 1
                ...
                filename=sprintf('A_WT_nomito',hertz(freq),mcu_syl);
            end
            clear x_s t_s x0
            tic
            ...
            %%%%preallocation if needed%%%%%
            if sampling_switch==1
                x_s=zeros((t_finish-t_step)*t_sampling,all_var);
                t_s=zeros((t_finish-t_step)*t_sampling,1);
            end
         ...
         avg=0;
            while t_end<=t_finish+t_step/2
                ...
                if t_ini==0
                    ...
                    x0(:)=x(end,:);
                    if sampling_switch==1
                        t_interp=t_ini-t_step:1/t_sampling:t_end-t_step;
                        range_interp=(ind_cycle-1)*t_sampling*t_step+1:(ind_cycle)*t_sampling*t_step+1;
                        t_s(range_interp)=t_interp;
                            
                        for i_interp=1:all_var
                            x_s(range_interp,i_interp)=interp1(t,x(:,i_interp),t_interp); 
                        end
                    else
                        n_p=2;   
                ...
                if t_ini>0
                    if sampling_switch==1
                        plot_range=1:(ind_cycle)*t_sampling*t_step+1;
                        plot(t_s(plot_range),mean(x_s(plot_range,1:m*(n-1)),2),'r',...
                            t_s(plot_range),x_s(plot_range,Mito_free)*10)
                        t_e=(ind_cycle)*t_sampling*t_step+1;
                        t_i=find(t_s>=t_ini-t_step+t_step/10,1);
                        mid=mean(mean(x_s(t_i:t_e,1:m*(n-1)),2));
                    else
